...
proc hl_alm::init {w font szfont args} {
  ...
  $w tag config tagKEY -font $font -foreground $clrCOM
  dict set font -weight normal
  $w tag config tagPATH -font $font -foreground $clrVAR
  ...
  foreach t {KEY PATH ARG CMNT} {after idle $w tag raise tag$t}
...
proc hl_alm::line {w {pos ""} {prevQtd 0}} {
  ...
  foreach t {KEY PATH ARG CMNT} {$w tag remove tag$t $il.0 $il.end}
  if {[regexp "^\s*#" $line]} {
    $w tag add tagCMNT $il.0 $il.end
    return yes
  }
  lassign [regexp -inline {^\s*[^\s]+} $line] lre
  if {$lre ne {}} {
    set p1 [string length $lre]
    $w tag add tagKEY $il.0 $il.$p1
  }
  set path [regexp -inline -all -indices {\s{1}[.]{1}[^\s]+} $line]
  foreach l2 $path {
    lassign $l2 p1 p2
    if {$p1<$p2} {
      $w tag add tagPATH $il.$p1 $il.[incr p2]
    }
  }
  ...
  foreach l2 $key {
    lassign $l2 p1 p2
    if {$p1<$p2} {
      $w tag add tagARG $il.$p1 $il.[incr p2]
    }
  }
  return yes
}
